...
const Hero = ({saveForComparison, setCurrentPage, unit, setUnit }) => {
  ...
  const [occupantNumber, setOccupantNumber] = useState(5);
  ...
  const [merv, setMerv] = useState(0.77);
  const [filter, setFilter] = useState(0.77); 
...
return (1 - Math.exp(-1 * Breathing * d_int)) * 100;
  ...
  const handleSubcategoryChange = (event) => {
    ...
    const newOutdoorAir = (floorArea * ASHRAE62ft + occupantNumber * ASHRAE62p) / 1;
...
onChange={event => {
  ...
  const newOutdoorAir = (newFloorArea * ASHRAE62ft + occupantNumber * ASHRAE62p) / 1;
  setOutdoorAir(newOutdoorAir);
}}
min="20"
max="100000"
...
/>
    ...
    <div className="graph-choice" style={{ display: 'flex', alignItems: 'center', marginLeft: '15px'}}>
      ...
      onChange={event => {
        const newOccupantNumber = event.target.value;
        setOccupantNumber(newOccupantNumber);
        const newOutdoorAir = (floorArea * ASHRAE62ft + newOccupantNumber * ASHRAE62p) / 1 ;
        setOutdoorAir(newOutdoorAir);
      }}
      min="1"
max="10000"
...
/>
  ...
  <option value="0">MERV 4</option>
  <option value="0">MERV 5</option>
  <option value="0">MERV 6</option>
  <option value="0">MERV 7</option>
  <option value="0">MERV 8</option>
  <option value="0">MERV 9</option>
  <option value="0">MERV 10</option>
  <option value="0.60">MERV 11</option>
  <option value="0.71">MERV 12</option>
  <option value="0.77">MERV 13</option>
  <option value="0.88">MERV 14</option>
  <option value="0.91">MERV 15</option>
  <option value="0.95">MERV 16</option>
  <option value="0.99">HEPA</option>
</select>
...
<div className="graph-choice" style={{ display: 'flex', alignItems: 'center', marginLeft: '15px'}}>
      <br/>
      <label htmlFor="hvacTreatment" style={{ fontSize: '0.9rem'}}>Air treatment ECAi (cfm): </label>
...
<label htmlFor="roomAC" style={{ fontSize: '0.9rem'}}>each ECAi (cfm): </label>
...
<label htmlFor="roomUV" style={{ fontSize: '0.9rem'}}>each ECAi (cfm): </label>
...
<div className="graph-choice" style={{ display: 'flex', alignItems: 'center', marginLeft: '15px'}}>
    ...
    <label  style={{ fontSize: '0.9rem'}}>Breathing rate: <br/> (m³ ∙ h⁻¹) </label>&emsp;
...
<div className="top-navbar">
  <a href="#ASHRAES" className={selectedTab === "ASHRAES" ? 'active' : ''} onClick={() => setSelectedTab("ASHRAES")}>ASHRAE 241</a>
  <a 
  href="#ASHRAE" 
...
>
  ASHRAE 241& Infection risk
</a>
</div>

<div className="top-navbar">
  
<a href="#Target" className={selectedTab === "LANCET" ? 'active' : ''} onClick={() => setSelectedTab("LANCET")}>The Lancet</a>
    ...
    <div className="card2">
      <div className={`result-container ${isCompliant ? 'compliant' : 'noncompliant'}`}>
        {isCompliant ? (
          ...
          <>
            <span>✗</span>&nbsp;
            Does not Complies with ASHRAE Standard 241
          </>
        )}
      </div>
      ...
      >
        Total ECAi: {totalCADR} cfm&emsp; ECAi: {ASHRAE} cfm/person&emsp;V<sub>ECAi</sub>: {ASHRAE * occupantNumber} cfm
      </span>
...
<span
  ...
  {selectedTab === "ASHRAE" && (
    ...
    <div className="card2">
      ...
      >
        Total ECAi: {totalCADR} {unitChoice}&emsp; ECAi: {ASHRAE} cfm/person&emsp;V<sub>ECAi</sub>: {ASHRAE * occupantNumber} cfm
        </span>
        <span
        style={{
          display: 'inline-flex',
...
{selectedTab === "LANCET" && (
    <>
    <div className="card2">
    <span style={{color: '#333333'}}>Compliance with the Lancet</span>
    <table>
    <thead>
      <tr>
...
{selectedTab === "Target" && targetType === "Individual risk (%)" && (
  ...
  typeD, typeInact, infilmin, infilmax, dmin, dmax, dmu, dsigma, inactmin, inactmax, inactmu, inactsigma
      ...
      >
        Total ECAi: {totalCADR} {unitChoice}&emsp;Individual risk: {parseFloat(risk_Calculation(VBmin, VBmax, Valpha, Vbeta, kmin, kmax, Kmin, Kmax, Cimin, Cimax,
  typeCv, CVmu, CVsigma, CVmin, CVmax,
  type111, type222, type333, type444, type555, 
  resting, standing, light, moderate, heavy, 
  BR1mu, BR1sigma, BR1min, BR1max, BR2mu, BR2sigma, BR2min, BR2max, 
...
<div className="graph-choice2">
  <label htmlFor="graphChoiceS" style={{fontSize: '0.9rem'}}> Chart: </label>
  <select
    id="graphChoiceS"
    value={graphChoiceS}
    onChange={(e) => setGraphChoiceS(e.target.value)}
  ...
  >
    ...
    <option value="Equivalent clean airflow vs. Outdoor air">
...
{selectedTab === "LANCET" && (
    ...
    <div className="graph-choice2">
      <label htmlFor="graphChoice2" style={{fontSize: '0.9rem'}}> Chart: </label>
      <select
        id="graphChoice2"
        value={graphChoice2}
...
<div className="graph-choice">
  <label htmlFor="graphChoice" style={{fontSize: '0.9rem'}}> Chart: </label>
  <select
    id="graphChoice"
    value={graphChoice}
...
<div className="graph-choice">
  <label htmlFor="graphChoiceT" style={{fontSize: '0.9rem'}}> Chart: </label>
  <select
    id="graphChoiceT"
    value={graphChoiceT}
